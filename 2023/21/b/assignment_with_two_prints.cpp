...
public:
  ...
  void printGrid2(const set<PIII> &seen, int maxSteps) {
    vector<vector<char>> grid2(grid.size(), vector<char>(grid[0].size(), '.'));
    ...
    for (int row = sRow; row < eRow; ++row) {
      for (int col = sCol; col < eCol; ++col) {
        ...
        if (seen.find({maxSteps, {row, col}}) != seen.end()) {
          // cout << "X";
          if (grid2[modRow][modCol] == '.') {
            grid2[modRow][modCol] = '1';
          } else if (grid2[modRow][modCol] == '9' || grid2[modRow][modCol] == 'X') {
            grid2[modRow][modCol] = 'X';
          } else {
            grid2[modRow][modCol]++;
          }
        } else {
