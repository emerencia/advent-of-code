...
public:
  ...
  void settleBricks() {
    ...
    for (auto &brick: bricks) {
      ...
      for (auto &support: brick.supports) {
        cout << (char) ('A' + support - 1) << " ";
      }
      cout << " supported by: ";
      for (auto &supportedBy: brick.supportedBy) {
        cout << (char) ('A' + supportedBy - 1) << " ";
      }
      cout << endl;
    }
    */
  ...
  int canBeDisintegrated() {
    int result = 0;
    for (auto &brick: bricks) {
      bool allAreSupportedByOtherBricks = true;
      for (auto &support: brick.supports) {
        auto &sBrick = findBrick(support);
        if (sBrick.supportedBy.size() == 1) {
          allAreSupportedByOtherBricks = false;
          break;
        }
      }
      result += allAreSupportedByOtherBricks;
    }
    return result;
  }
