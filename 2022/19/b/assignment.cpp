...
public:
  int geodesMined(const Blueprint &blueprint) {
    ...
    set<State> seen;
    // timeLeft oreRobots clayRobots obsidianRobots geodeRobots oreCount clayCount obsidianCount geodeCount
    q.push(make_tuple(32, 1, 0, 0, 0, 0, 0, 0, 0));
    while (!q.empty()) {
      State p = q.front();
      q.pop();
      ...
      // Reduce ore count (if we have more than we can spend in the remainder of the session)
      if (oreCount > timeLeft * (blueprint.maxOreCost - oreRobots) + oreRobots) {
        oreCount = max(0, timeLeft * (blueprint.maxOreCost - oreRobots) + oreRobots);
      }
      if (clayCount > timeLeft * (blueprint.obsidianRobotClayCost - clayRobots) + clayRobots) {
        clayCount = max(0, timeLeft * (blueprint.maxOreCost - clayRobots) + clayRobots);
      }
      if (obsidianCount > timeLeft * (blueprint.geodeRobotObsidianCost - obsidianRobots) + obsidianRobots) {
        obsidianCount = max(0, timeLeft * (blueprint.geodeRobotObsidianCost - obsidianRobots) + obsidianRobots);
      }
  ...
  int solution() {
    ...
    */
    for (int i = 0; i < 3; i++) {
      result *= geodesMined(blueprints[i]);
    }
    return result;
  }
};
